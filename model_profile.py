...
from acktr.model import Flatten, NNBase, Policy
...
def _clean_state_dict(state_dict):
    ...
    # Legacy checkpoints may store AddBias params as bias tensors with shape (N, 1).
...
def _infer_model_config(state_dict, container_height):
    ...
    if 'base.share.0.weight' in state_dict:
    ...
    if area is None or hidden_size is None or action_dim is None:
        raise KeyError('Missing key for config inference. Need area/hidden_size/action_dim from checkpoint.')
    ...
    if channel is not None:
        ...
        raise KeyError('Cannot infer obs length from checkpoint keys.')
...
class LegacyCNNPro(NNBase):
    def __init__(self, num_inputs, recurrent=False, hidden_size=256, args=None):
        super(LegacyCNNPro, self).__init__(recurrent, num_inputs, hidden_size, args)
...
def _select_base_class(state_dict):
    # Legacy checkpoints contain mask head weights in base.mask.*
    if 'base.mask.5.weight' in state_dict or 'base.mask.2.weight' in state_dict:
        return LegacyCNNPro
    return None
...
def main():
    ...
    parser.add_argument('--model-path', type=str, default='pretrained_models/cut1.pt', help='Path to model checkpoint')
    ...
    selected_base = _select_base_class(state_dict)
