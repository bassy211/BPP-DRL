...
def get_args():
    ...
    parser.add_argument(
        '--algorithm', default='acktr', type=str,  help='algorithm used, acktr|ppo|a2c|random|first_fit|best_fit|corner_point|extreme_point|ems|macs'
    )
    parser.add_argument(
    ...
    if args.mode == 'test' and not args.load_model:
        if args.algorithm not in ['random', 'first_fit', 'best_fit', 'corner_point', 'extreme_point', 'ems', 'macs']:
