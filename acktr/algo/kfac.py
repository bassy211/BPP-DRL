    ...
    def step(self):
        ...
        for i, m in enumerate(self.modules):
            ...
            if self.steps % self.Tf == 0:
                ...
                eps = 1e-6
                m_gg_reg = self.m_gg[m] + eps * torch.eye(self.m_gg[m].size(0), device=self.m_gg[m].device)
                m_aa_reg = self.m_aa[m] + eps * torch.eye(self.m_aa[m].size(0), device=self.m_aa[m].device)
                ...
                try:
                    ...
                    U_g, S_g, _ = torch.linalg.svd(m_gg_reg)
                    self.d_g[m], self.Q_g[m] = S_g, U_g
                    U_a, S_a, _ = torch.linalg.svd(m_aa_reg)
